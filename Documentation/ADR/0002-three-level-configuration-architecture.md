# ADR-0002: Three-Level Configuration Architecture (Provider → Model → Configuration)
...
1. **Single Source of Truth:** API key stored once per provider
2. **Flexible Connections:** Multiple providers of same type (prod/dev/backup)
3. **Model Catalog:** Centralized model specs and pricing
4. **Clear Separation:** Connection vs capability vs use-case concerns
5. **Easy Key Rotation:** Update one provider, all configs inherit
6. **Cost Tracking:** Usage attributable to specific providers
7. **Multi-Tenancy Ready:** Different API keys per team/project

### Negative

1. **Increased Complexity:** Three tables instead of one
2. **More Joins:** Queries must traverse relationships
3. **Migration Required:** Existing data needs transformation
4. **Learning Curve:** Users must understand hierarchy
...
```php
class ThreeLevelMigrationWizard implements UpgradeWizardInterface
{
    public function executeUpdate(): bool
    {
        // 1. Create providers from unique (adapter_type, api_key) pairs
        // 2. Create models from unique (provider, model) pairs
        // 3. Update configurations to reference models
        // 4. Migrate prompts and parameters
    }
}
```
...
- Old `provider` and `model` string columns kept temporarily
- New `model_uid` column references Model table
- Upgrade wizard populates relationships
- Remove old columns in future version
...
- [Original Plan File](../.claude/plans/smooth-mixing-cerf.md)
- [TYPO3 Extbase Domain Modeling](https://docs.typo3.org/m/typo3/reference-coreapi/main/en-us/ExtensionArchitecture/Extbase/Reference/Domain/Model.html)
