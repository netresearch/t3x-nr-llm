...
- **The default-PHP Rector run is mostly noise, but not only noise — read the rule names, do not dismiss the count.** Measured on `main`: `runTests.sh -s rector -n` without `-p` reports 59 files, and the rules behind them are `RenameMethodRector` (54) and `AllowMockObjectsForDataProviderRector` (11) — both bound to the phpunit the default resolves, which is 13, and both invisible to the 8.2 leg. What makes that dangerous is the obvious next thought: "the default run is noise, so the gate is noise". It is not. On #906 the same tree reported one finding under `-p 8.2` that was genuine and had to be fixed. Treat a default-PHP count as unclassified, not as clean.
