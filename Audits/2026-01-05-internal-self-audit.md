...
>
> This report described nr_llm as it stood on 2026-01-05. It sat at the
> repository root as `SECURITY_AUDIT.md` until 2026-08-10 and was archived
> here unchanged below the line, because parts of it no longer describe the
> code and its own review date had lapsed.
>
> For the current assurance surface, read
> [`SECURITY_AUDIT.md`](../SECURITY_AUDIT.md) in the repository root.

## What expired in this report

| Statement below | Reality since |
|---|---|
| `Classes/Service/ApiKeyEncryptionService.php` audited as "Verified Secure" | The class does not exist. API keys are nr-vault UUID identifiers under nr-vault's envelope encryption (ADR-012); no `sodium_crypto_*` call remains in `Classes/`. |
| "API key encryption: sodium_crypto_secretbox" in the configuration table | Same — the audited implementation was removed, not re-audited. |
| "Recommendation: Ready for production" | A point-in-time statement about a 2026-01 tree, carried without re-verification across ~25 minor releases. |
| "Next Audit Due: 2026-07-05" | Not performed. The date lapsed with the report still presented as current. |
| "PHPStan Level 9" | Level 10. |
| "Lockfile committed and maintained (composer.lock)" | `composer.lock` is deliberately **not** committed — nr_llm is a library and the lock file would conflict with project-level resolution. |
| "0 critical / 0 high / 0 medium" over a full scope | Later targeted audits did find real issues in code written after this date — see the 2026-07-19 tool-security review, whose confirmed findings are fixed and merged. The zero counts describe January's scope, not the extension's history. |

The rest of the report is preserved verbatim for the record.
