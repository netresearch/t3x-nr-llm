...
- **`create_translation_draft` machine-translates the text of the translation (ADR-209, NEXT-166).** After core's `localize` created and hid the record, every `input` and `text` column of the record's type — the columns of the type's form with its `columnsOverrides`, without `l10n_mode = exclude`, `defaultAsReadonly`, `readOnly`, a `text` column rendered in a code editor, or `pages.author` — that holds text in the source is translated from the default-language record (so the "[Translate to …:]" prefix never reaches the translator). Rich text is sent as HTML. The new optional `translator` argument chooses `deepl` or `llm` (default `llm`, TranslationService's own default without a configuration); a translator the installation has not configured, and a target language the record's site does not define (or a page in no site, each with its own reason), are refused before anything is created — the language check was core's, after the record existed. The site of the record's page selects the site glossary (ADR-208). The approval preview names the translator, the language pair, the fields, the fields the user may not edit, and whether the site keeps a glossary for the pair. All fields are translated before anything is written: a translator failure, a blank answer or one cut off at the output limit writes nothing, and the result says *The text was NOT machine-translated* with the reason. The translations are then written in one DataHandler pass and read back; a value longer than an `input` column's TCA `max` is cut to it and named; if the write fails — a DataHandler error or an exception from a hook — the result names which fields hold the translation and which still hold the source text, and a failure reported after every field was stored is named with the success. The result always carries the write target, so `AfterAiRecordWrittenEvent` announces the created record. A transparency-notice field for machine-translated records was not added: this extension does not extend `pages` or `tt_content`, and ADR-187 ships no listener; ADR-209 records why.

- **Opt-in translation cache and markup handling in `TranslationOptions` (ADR-209, NEXT-166).** `withCacheTtl(int)` caches the result of `TranslationService::translateWithTranslator()` in `nrllm_responses` (tag `TranslationService::CACHE_TAG`, `nrllm_translation`), for DeepL and the LLM translator alike. The key is the translator, both languages, the text and the final translator options — including the site glossary's terms or DeepL glossary id, so an edited glossary is a new key — and, for the LLM translator, what the options do not name: without a pinned provider (model pinned or not) the default configuration the chat call resolves — uid, identifier, model, last change and the identifier and body of each skill it composes into the prompt — and with a pinned provider but no model the provider's default model; when that cannot be resolved the call is not cached. `tx_nrllm_configuration` now declares `tstamp` as a `passthrough` TCA column, so `LlmConfiguration::getTstamp()` returns the stored value instead of 0. The attribution fields are not part of the key. A hit skips the translator, its budget pre-flight and its usage row, carries `metadata['cached'] = true` and `charactersUsed = 0`; a failure, a blank answer and a truncated one are never stored; without a TTL nothing changes. Saving or deleting a glossary, configuration, model, provider, skill or prompt snippet flushes the tag (new DataHandler hook `TranslationCacheFlushHook`). `withTagHandling('html'|'xml')` reaches DeepL as `tag_handling`. Both options are appended to the constructor with getters; `toArray()` emits `tag_handling`. `TranslationService`'s constructor gains two optional trailing parameters, `CacheManagerInterface` and `ConfigurationResolver`; positional callers are unaffected.

### Changed

- **`LlmTranslator` sizes its output budget to the text and reports truncation (ADR-209, NEXT-166).** Without an explicit `max_tokens` and without a pinned provider it asks for one token per UTF-8 byte of the text, at least the former fixed 2000 and at most 16000, so a long text is no longer cut off at 2000 tokens; with a pinned provider, which runs without a model record and so without a known output limit, the former 2000 stays. Its result metadata carries `finish_reason` and `truncated` (`true` for `length`).

- **Configuration-driven calls cap `max_tokens` at the model's output limit (ADR-209, NEXT-166).** `ConfigurationCallPlanner::callOptions()` caps whatever `max_tokens` wins — an explicit per-call value or the configuration's stored one — at the model's `max_output_tokens` when that is known (> 0). A provider refuses a larger value with an error; a caller that asked for more now gets the model's limit instead.
...
- **The test page shows the DeepL character quota (ADR-207, NEXT-166).** The translation card of *LLM > Overview > Test* now shows the characters used and the limit of the current billing period, the share used, and whether the key is a Free key, a Pro key or a custom endpoint. It is read from DeepL's usage endpoint, which costs no characters, when the page opens and after each DeepL translation. Without a DeepL key the line says so and names the Extension Configuration; a refused key is reported as refused; the key never reaches the page. New AJAX route `nrllm_test_deepl_quota`; `DeepLTranslator` implements the new `CharacterQuotaReportingInterface` (`getCharacterQuota(): CharacterQuota`). `DeepLTranslator::getUsage()` had no caller before.
...
- **The read tools say which language a record is in (NEXT-167).** On the demo a page and its translation share a title and sit one uid apart, and the model asked "which of these two pages?" fourteen times in one conversation. `read_records` always returns the table's language field and translation parent (TCA `languageField`, `transOrigPointerField`) on a language-aware table, `search_records` names each hit's language and the record it translates, `get_page_content` names the page's language and translation parent, each translated element's source, and the other languages that hold content on the page (only those the acting user may access), and `get_pagetree` states that it lists default-language pages only. The `read_records` table artifact gains the same two columns. `get_page_content` called with a translated page's uid now lists the content of the page it translates, where translated elements live, instead of reporting an empty page; elements for all languages (`-1`) are not named as another language, because no call can list them.

- **`create_page_draft` leads its result with the new uid and warns about a same-titled sibling (NEXT-167).** Twice on the demo the element meant for a freshly created page landed on its parent. The result now starts with `New page uid: N.` and names the parent as the page NOT to use. The approval preview gains a first line when the parent already holds a default-language page with the same title (the database's comparison, so case-insensitive on MySQL/MariaDB), hidden ones included, and the acting user may see that page: after a lost answer, "weiter" made the model draft the same page again and both copies were approved (demo conversation 84). It warns and does not refuse. When two runs draft the same page, the second approval is handed back by ADR-184's preview comparison with the warning in the fresh preview.

- **Two tool descriptions state their limits (NEXT-167).** `update_page_metadata` says it writes text fields only and cannot set an image, hide, unhide or move a page or change its content; a call naming `og_image` or `twitter_image` is refused with a pointer to `set_page_social_image` (the model had promised the user those fields). `create_translation_draft` says it takes no translated text.

### Added

- **`UnavailableToolsResolverInterface`: the tools a run will not be offered, with the reason (ADR-201, NEXT-167).** New `@api` service, public in the container: it runs the composite gate (ADR-094) over every registered tool for one configuration and one backend user and returns the refusals as `ToolPolicyDecision`s — `toolDisabled`, `requiresAdmin`, `configurationGroup` or `trustZone`. A consumer can hand the list to the model, so it can say that a tool exists but is not enabled for this user instead of claiming nothing can do it (demo conversation 104). A builtin tool's trust-zone refusal in observe mode is offered and therefore not listed; a remote (MCP) tool is enforced in both modes and is listed. Remote tools are listed for administrators only. `ToolCallPolicyInterface::explain()` and `filterOfferable()` now resolve the enabled set and the configuration's allow-list once per call instead of once per tool. Additive; the API surface snapshot gains the interface and the four denial constants on `ToolLoopServiceInterface`.

- **The assistant can read a public web page on another site (ADR-202, `fetch_external_url`, NEXT-165).** In the demo chats users asked five times for an external page to be read or analysed; `probe_url` reaches only the installation's own hosts and `site_fetch_source` only its own search. The new read tool fetches one http(s) URL and returns title, headings, main text and links as plain text, between `<<<BEGIN/END UNTRUSTED EXTERNAL WEB CONTENT>>>` markers with a note that it is reference material, not instructions; a copy of a marker inside the page is defused. Every URL and every redirect target passes an address guard: http(s) only, no credentials, default ports, no legacy numeric host forms (`2130706433`, `0177.0.0.1`), and every address the host resolves to through DNS must be public — private, loopback, link-local, CGNAT, multicast, reserved and metadata ranges are refused, IPv4-mapped, 6to4, Teredo and NAT64 forms of them included, and a host DNS does not answer is refused. The connection is pinned to the checked addresses with `CURLOPT_RESOLVE`, so a DNS answer that changes before the connect cannot redirect it, and it goes through nr-vault's hardened client. At most three redirects, 2 MiB downloaded (the transfer is aborted, not only truncated), 20,000 characters returned (and the whole result kept under 48,000 bytes, so the loop's result bound never cuts the END marker off), 5 s to connect and 20 s in total; only HTML and plain text are read. The tool is the only member of a new tool group `web` with a new egress scope `external_filtered`, so `system` and `probe_url` keep their own-site egress; it ships disabled and is not admin-only. Every call waits for a human approval, because the URL itself carries data out; the approval card shows the host and the query string, and `tools.fetchExternalUrl.skipApprovalWithAllowlist` lifts it only while a host allowlist is set (the new `ConfigurableApprovalInterface`, which cannot lift the approval of a declared write). Hosts of this installation are refused, and so is any fetch an HTTP proxy would carry (TYPO3 proxy setting, `HTTPS_PROXY`, `HTTP_PROXY` on the CLI, honouring `NO_PROXY`) unless `tools.fetchExternalUrl.allowViaProxy` is set together with an allowlist; without ext-curl nothing is fetched. The HTML parser gets at most 256 KiB, and markup nested deeper than 200 levels is reduced to its text without parsing (masterminds/html5 is quadratic in the depth); the time budget is checked before every DNS lookup and before parsing. The fence markers carry a random nonce per call, and marker look-alikes in the page are defused; error results repeat no server-chosen text except a redirect target, cut to 500 characters and labelled untrusted. The host lists `tools.fetchExternalUrl.allowedHosts` and `tools.fetchExternalUrl.deniedHosts` take names and IPv4/IPv6 addresses (an address entry also matches what a name resolves to); neither can admit a private address. `masterminds/html5`, which TYPO3 already installs, is now a declared dependency for the HTML parsing.

- **Editorial Starter ships five more tasks (NEXT-174, NEXT-166).** The pack had summarise, rewrite, proofread and headlines; the demo guide promised "expand" and "fix grammar", which existed only in cowriter's development seed. New: *Expand* (develops a text without adding facts), *Correct spelling and grammar* (returns the corrected text, where *Proofread* only lists corrections), *Plain language version* (Einfache Sprache), and two review tasks that change nothing: *Check readability* and *Check for gaps and tone* (against the house-style snippet). An installation that already has the pack gets them by installing it again: the installer skips the identifiers it has.

- **Reasoning effort is a request option, and a turn can carry its provider's own items (ADR-203, ADR-204, #965).** New `@api` enum `ReasoningEffort` (`none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, the values OpenAI's `reasoning.effort` defines). `ChatOptions::withReasoningEffort()` and `getReasoningEffort()` carry it to the provider as the options key `reasoning_effort`; it is a fluent setter rather than a constructor parameter because `ToolOptions` repeats its parent's thirteen parameters positionally, so a fourteenth would move `toolChoice` for every positional caller. `ToolOptions::fromArray()` restores it, so a resumed run keeps the effort it was suspended with. `ChatMessage` gains an optional trailing `?array $providerItems` (also on `assistantToolCalls()`) and a `toTranscriptArray()`: the provider's opaque response items for one turn, stored and replayed untouched. `toArray()` does **not** carry them, because it is the Chat Completions wire shape and the Groq, Mistral, OpenRouter and Ollama adapters send it to a live API as it is; `toTranscriptArray()` is the stored shape, `fromArray()` reads the key back, and suspended and queued runs are now persisted in that shape. All of it is additive in the API surface snapshot.
...
- **BEHAVIOUR: tool calling works on OpenAI's GPT-6 models (ADR-203, #965).** `OpenAiProvider::chatCompletionWithTools()` posted every tool request to `chat/completions`, and OpenAI refuses function tools there for a GPT-6 model at its default reasoning effort: Sol and Luna accept them only with `reasoning_effort: none`, and Astra accepts them under no setting, since it has no `none` effort at all. The request failed before a tool ran, in the Tool Playground and in every agent run. A tool request to a model that cannot take tools on `chat/completions` now goes to `/v1/responses` instead: tool definitions in the flat Responses shape and explicitly non-strict, as they are on Chat Completions, text and image parts as `input_text` and `input_image`, a system prompt as `instructions` (several joined, not the first kept), `response_format` as `text.format`, and `store: false`, so no conversation is retained on OpenAI's side. Across the steps of one run the reply's own `output` items — the reasoning item with its `encrypted_content`, the `function_call` item, the visible message — are replayed into the next request untouched, and the tool result goes back as a `function_call_output` with the `call_id` the call carried; that survives an approval pause and the run queue, because a suspended or queued run is now stored in `ChatMessage::toTranscriptArray()` shape and the tool loop turns such a stored turn back into a `ChatMessage` before anything is sent, so the items never reach a Chat Completions request. Unchanged: every other model, including `gpt-4.1-mini`, the GPT-5 and the o-series reasoning models, stays on `chat/completions`; so do plain chat, streaming and vision; and so does any endpoint that is not `api.openai.com`, because an OpenAI-compatible gateway need not serve `/v1/responses` — a configuration opts in with `"openai_tools_transport": "responses"` in its options JSON, and can force the old path with `"chat/completions"`. Which transport served a call, the effort applied and the items to replay are written to `CompletionResponse::$metadata` under the keys in `OpenAiCallMetadata`. No request to OpenAI was made while this was built — no credential for it was available — so the shapes are the published API reference's, and the first real call is the measurement.

- **BEHAVIOUR: the Tool Playground's "Thinking" switch does something on OpenAI (ADR-204, #965).** `ChatOptions::$think` was read by the Ollama adapter alone, and `getThink()` had no caller, so "Thinking: Off" changed nothing on the six other providers. On OpenAI `think = false` now selects the lowest effort the model allows: `none` on GPT-6 Sol and Luna, `low` on GPT-6 Astra, which rejects `none` with HTTP 400. The applied value is recorded on the response together with whether the provider reported it or it is what was sent, so a clamped request is readable rather than silent. An explicit `reasoning_effort` wins over `think`. A plain completion now carries the effort too. GPT-5 and o-series models get no effort, as before: their scales differ inside each family (o1-mini takes none at all, o1 and o3 stop at `high`) and were not read for this change, so neither option sends anything to them and no effort key is recorded. Claude, Gemini, Groq, Mistral and OpenRouter still ignore both options and write no effort key, which is how that shows.

- **GPT-6 models are listed, and are no longer sent sampling parameters (ADR-203, #965).** The setup wizard's relevance filter had no `gpt-6` pattern, so live discovery dropped `gpt-6-astra`, `gpt-6-sol` and `gpt-6-luna`; they are now listed with the context window, output limit and prices of their model pages, and appear in the fallback catalogue and in `getAvailableModels()`. `isReasoningModel()` matched `/^(o[1-9]|gpt-5)/`, so every GPT-6 request carried `temperature`; the answer now comes from one model-profile table that also decides the transport and the effort scale. No documentation page says GPT-6 rejects a non-default `temperature`, and it was not measured: if GPT-6 in fact accepts one, a configured temperature is now dropped for these three models without an error, and this is the line to revisit.
...
- **OpenSSF Baseline now shows the live assessment status.** The README uses the official badge, and `BASELINE.md` maps every Level 1 control in v2026.08.28 to evidence or an explicit open review. The document distinguishes configured approvals from human review and required checks from advisory reports. Two generated Python bytecode files were removed; their `.py` sources remain, and Python caches are ignored.
...
- **`create_content_element_draft` offers the TCA's content types under an exclusion rule instead of a fixed four (ADR-196, NEXT-160).** The tool shipped with a static allow-list — `header`, `text`, `textmedia`, `bullets` — and a fixed field set; on the Netresearch demo, whose site package declares twenty-six content types, an assistant could create four of them and had to refuse a table or a card. The offered types are now every `tt_content.CType` item that is not denied by name (`list`, `html`, `shortcut`, `div`, `menu_*`) and whose form — the `showitem` with palettes expanded, `columnsOverrides` applied — holds no column whose payload is not prose: `flex`, `inline`, `group`, `folder`, a `foreign_table` select, `slug`, `password`, and every TCA type the tool does not know, which fails closed. `file`, `category` and `link` columns do not exclude a type and are never filled; core puts `categories` on every content type and `header_link` in the header palette every type uses, so any rule under which they excluded would drop the four original types too — `textmedia` is offered as before and gets its media through `attach_file_to_content_element`. A new optional argument `fields` carries the chosen type's further scalar columns (`input`, `text`, `select` with static items, `check`, `number`, `datetime`, `radio`, `color`, `email`), each validated against its TCA type: a select against its items, a number within its range (the DataHandler would clamp in silence), a check as a boolean, an input within the TCA `max`. Identity, position, visibility, publication, audience and translation columns are refused by name, `header` and `bodytext` stay arguments of their own — `bodytext` refused for a type whose form does not show it, where the DataHandler would write it under the base config and an RTE base would make the read-back take a correct element back — and the first wrong key or value refuses the whole call before anything is written, naming the columns the type offers. The exclude-field grant is asked per column before the write, as `update_fal_asset_meta` asks it, and the read-back checks every column the call set, deleting the element again when one did not take. The JSON schema declares `fields` as an object with free keys (the boolean `additionalProperties` form `read_records` ships, since a type array is a union Gemini's schema dialect does not express) and names the TCA as their source; it does not enumerate the columns of every type. On a stock installation core's `table` becomes reachable; `uploads`, `shortcut`, `div`, `html` and the menus do not. A plugin is excluded like the deny-list, because `ExtensionManagementUtility::addPlugin()` gives one without a FlexForm `header`'s scalar form: a `CType` is a plugin when Extbase registered it (`ExtensionUtility::configurePlugin()` records it under `EXTCONF.extbase.extensions.<Extension>.plugins.<Plugin>` and the content type is `strtolower(<Extension>_<Plugin>)`, the same in 13.4 and 14.3) or when its item sits in the `plugins` or the `forms` group — `registerPlugin()` defaults to `plugins`, and core registers indexed_search, felogin and form into `forms`, where `indexedsearch_pi2` has no FlexForm. A non-Extbase plugin in another group is judged by its form. Where `CType` declares `authMode`, the chosen type is checked against the acting user's `explicit_allowdeny` before the write, because the DataHandler drops a disallowed type in silence and creates the element as the default type. The grant check covers the columns the tool writes itself (`hidden`, `header`, `bodytext`, and `colPos` and `sys_language_uid` where they differ from `0`) and reads `exclude` as a boolean cast, as core's schema does; the read-back compares position and language as well. A `datetime` is handed over as the integer both supported cores store verbatim — a Unix timestamp, or seconds of the day on a `time` column — because 13.4 reads an ISO string as UTC wall time and shifts it by the server's offset; a native `dbType` column takes unqualified local wall time, and the approval card shows the day or the time of day the integer stands for, not the integer. An `input` or a non-RTE `text` below the TCA `min` (a bound given as a numeric string counts), a colour with an alpha pair on a column without `opacity`, a select value outside the acting user's `explicit_allowdeny` where the column declares `authMode`, a decimal with more than two places (the DataHandler stores two), and a decimal the DataHandler would clamp — it rounds to two places and then compares rounded up against the upper bound and rounded down against the lower — are refused, as the DataHandler would store `''`, a cut, dropped or clamped value; a multi-item `check`, a `check` with `eval` `maximumRecordsChecked(InPid)` and an `input` or `email` with `eval` `unique(InPid)` stay in the form and are refused as keys, because TYPO3 changes them depending on other records; the read-back refusal names a rewrite by TYPO3 as a cause beside a missing grant; an `input` with an `eval` beyond `trim` is read back for presence, and a decimal as a number. Page TSconfig `TCEFORM.tt_content` is honoured after the page is authorised, as FormEngine honours it and the DataHandler does not: a type `CType.keepItems`/`removeItems` takes out, a column `disabled` hides or `config.readOnly` locks, and a select item `keepItems`/`removeItems` removes — at column level or under `types.<CType>`, and for the `column` and `language` arguments (`colPos`, `sys_language_uid`) as well as for `fields` — are refused with the rule named, core's own `imageorient.types.image.removeItems` included; a `header` the page hides or locks refuses every call of that type there, since `header` is a required argument. A column that is `readOnly` in the TCA is refused as a key, and a TSconfig `config.readOnly = 0` does not lift that; `addItems` is not read. The type list in the description stays the TCA-level set.

### Added

- **The assistant can act on existing pages and content elements (ADR-198, ADR-199, NEXT-164).** NEXT-160 had kept updating, publishing and deleting out of reach as a security line to decide separately; the product owner decided on 2026-09-23 to include them, and ADR-198 records the decision and its rails. Six new writing tools, each disabled by default, in the `editing` group, `IDEMPOTENT_WRITE` or `NON_IDEMPOTENT_WRITE`, paused for a human approval on every call, writing through the DataHandler as the acting backend user in the live workspace only: `update_content_element` sets the scalar fields of one existing element — the field set `create_content_element_draft` offers for the element's type, checked by the same ADR-196 rules, which move unchanged into a shared `ReadsContentTypeFormsTrait` — and refuses an element of an excluded type (raw HTML, plugins, menus, shortcuts, FlexForm or inline forms), a column a translation takes from its default-language element, and what the page's TSconfig takes out of the form; the approval card shows every column's change bound to the whole value (below). `publish_record` clears the hidden flag of one page or element and nothing else, and names the start or stop time, access groups or hidden default-language record that still restrict it. `delete_record` deletes one page or element, recoverably, and counts what core deletes with it — translations, a page's records and its branch — (the subpage uids, their translations, and the records on the pages counted table by table — counts only, which include records the acting user cannot see) and the records that still point at it; a page with subpages is refused unless `include_subpages` is set, a branch over 50 pages and a site root always, and a delete that would take a workspace draft along is refused too — core discards the versions (a move version on another page included) and new workspace translations of every live row it deletes, its inline and file children included, and strands the other drafts on a deleted page; the tool counts both, and refuses when a table cannot be asked. `copy_record` copies one element or page with core's copy command — the copy and every translation copied with it always hidden, whatever `hideAtCopy`, the user's preferences and page TSconfig say — and a page always without its subpages (ADR-199); which translations are copied is core's decision and changed in 13.4.25 — from then on core asks the target site and copies none outside one, before it copied every page translation and dropped an element translation the target page is not translated into — the card states the running core's rule and the answer counts them. `move_page` moves one page to a new parent or after a sibling with the permissions core's `moveRecord()` asks; its card counts the subpages, says that the slug is not regenerated and warns when the page moves into another site, into a site or out of every site. `copy_record` and `move_page` refuse a page translation as target or parent. `replace_file_reference` points one file reference on a content element at another existing file at the same position, or removes it; nothing of the old reference — title, alternative text, description, crop — is carried over, and the translated references core deletes with it are named on the card, refused where the acting user may not change their translated element, and settled afterwards (each translated element's reference count set and read back). Every tool asks the acting user's page permission for its act, the record-level rights core asks (`tables_modify`, language, `authMode`, `editlock`), the languages of translations core carries along, and the field-level grant for every column it sets, before the write; it reads the result back after it. None declares an editor action: from a record's context menu and in bulk, a delete would be one click, which is a decision of its own. **BREAKING for exhaustive matches:** `WriteKind` gains `DELETED` for the first deleting writers — additive in the API surface snapshot, but a consumer that `match`es the `@api` enum exhaustively without a default arm now fails on a deletion and needs an arm for it. Every surface that counts the tools counts 57 tools and sixteen writers.

- **A record in an extension table can be drafted where no dedicated writer exists (ADR-197, `create_record_draft`).** On the Netresearch demo an assistant recognised EXT:news and its table and could not create a news record: every writer is narrow (ADR-135), and no narrow writer will exist for every extension an installation carries. The new builtin creates ONE hidden record in a TCA table by exclusion rather than by allow-list, and every exclusion is part of the decision: `pages` and `tt_content` have writers of their own and are refused by name; `sys_*` tables, the read-side denylist (`TableReadAccessService`) and tables declared `adminOnly`, `hideTable` or `readOnly` are out; a table another registered tool declares through the new `RecordCreatorInterface` (below) is refused and the tool named — read at call time from the `nr_llm.tool` tagged set, so an extension that ships its own creator withdraws the fallback the day it is installed, without a release here, and a declaration that cannot be read refuses the call; and the new extension configuration `tools.createRecordDraft.deniedTables` (comma-separated, empty by default) narrows the list per installation and can never widen it, refusing every table when it is present but cannot be read. Only scalar columns the record type's showitem lists — palettes expanded — can be set (`input`, `text`, `number`, `email`, `color`, `datetime` as a timestamp, `check`, `radio`, `select` with static items); relations, files, FlexForms, links and slugs are not arguments, and `hidden`, `uid`, `pid`, the language, timing, ownership and versioning columns are refused by name. Values are checked against the record type's live TCA, `columnsOverrides` included, before anything is written — the items of a select the way ADR-194 checks one, `max`, `range`, 0/1 for a check — and every column the record type marks required must be given, because the DataHandler drops an empty required value and clamps an out-of-range one in silence. A value the DataHandler would rewrite is refused as well: an `eval` token the DataHandler acts on (a token it ignores, such as a legacy `required` left in a `columnsOverrides` eval, is ignored here too), an eight-digit colour without `opacity`, a text below its `min`, a decimal with more than two decimal places (the DataHandler stores every decimal through `number_format($value, 2)`, so 1.234 would silently become 1.23), a decimal the range check would clamp by rounding. So is a column the backend form shows read-only on that page, which the DataHandler stores all the same — declared `readOnly` in the TCA, unless page TSconfig `TCEFORM.<table>.<column>.config.readOnly = 0` lifts it as it does in the form, or made read-only through that page TSconfig — and what the page's TSconfig takes out of the backend form, which the DataHandler does not enforce either: a `TCEFORM` `disabled` column, a select value outside `keepItems` or inside `removeItems`, a record type the type field does not offer there — the refusal names the rule. The record is always hidden and always in the default language: a translation needs a parent and a tool of its own, and a standalone record in another language is what ADR-193 found to mix a page. Rights are the acting user's, asked before the write — `tables_modify`, `Permission::CONTENT_EDIT` on the pid (what `DataHandler::hasPermissionToInsert()` asks for every table but `pages`), the field-level grant for every column the call sets (any truthy `exclude` flag), the hidden column included — and verified after it: what TYPO3 still rewrites in silence, a hook of the installation, is read back column by column — the forced default language and the record type included, which is resolved the way the DataHandler resolves it (the call's value, else `TCAdefaults` from the page's and then the user's TSconfig, else the column's default) and written explicitly only where the acting user may write the type column (its field-level grant, and on an `authMode` select the grant for that value); otherwise it is left to the DataHandler, which drops it in silence, and compared only where the DataHandler stores the resolved type itself (`TCAdefaults`, the TCA default) — the record is deleted again and the columns are named. The approval card names each column with the record type's TCA label — a showitem `field;Label`, else a `columnsOverrides` label, else the column's own; a page TSconfig `TCEFORM` label override is not applied, because the card must not depend on the page or the viewer's language — in English and shows a timestamp as an ISO 8601 date-time in UTC and a select value with its item label, so it reads the same for every viewer when the run resumes (ADR-184). It declares no editor action: it has no subject record an editor would select.

- **`RecordCreatorInterface`: a creating tool declares the tables it creates records in (ADR-197).** New `@api` extension point in `Netresearch\NrLlm\Service\Tool`: `getCreatedTables(): array` lists the tables a tool's records land in, which is not what `EditorAction::$recordTypes` names — that is the subject an action's arguments identify (ADR-152), so a content-element creator declares `pages` there. `create_record_draft` steps back from exactly the tables such a declaration lists. `create_content_element_draft` (`tt_content`), `create_page_draft` (`pages`) and `create_translation_draft` (`pages`, `tt_content`) implement it. Additive; the API surface snapshot gains the interface.
...
- **A page's social preview image can be set at all (ADR-195, `set_page_social_image`).** ADR-135 kept `og_image` and `twitter_image` out of `update_page_metadata` because they are FAL relations, and that left an assistant able to write a page's Open Graph title and description while telling the editor to pick the image by hand. The ninth writing tool references one **existing** `sys_file` from one default-language page's `og_image` or `twitter_image` — both columns come with EXT:seo, which is now a development requirement and a `suggest` entry — on the terms the previous eight meet: disabled by default, in the `editing` group, `NON_IDEMPOTENT_WRITE`, a human approval before every call, a preview at suspend that shows the file referenced now and the file that would be, a write through the DataHandler as the acting user, a read-after-write verification that takes the reference back when it fails — and then reads the page once more, so the message says whether it is back as the call found it or what is still there, and names what TYPO3 reported while putting it back — and a refusal vocabulary that never confirms a uid exists. It never uploads, moves or renames a file; the file must lie in a permitted storage inside the acting user's own file mounts and carry an extension the column's live `allowed` list accepts. **An image that is already there stops the call**: the refusal names the file referenced now, and `"replace": true` is the only way past it — it deletes that reference through the DataHandler (recoverably, `deleted = 1`) in the same run that creates the new one, and the approval card says so on a line of its own, the shape `create_translation_draft`'s `overwrite` already has. `typo3/cms-seo` declares no `maxitems` on either column, so the one-reference-per-field rule is this tool's decision, argued in the record, not a constraint read from the TCA. And the DataHandler checks a `sys_file_reference` row against `PAGE_EDIT` only while a `pages` row travels in the same datamap, against `CONTENT_EDIT` otherwise — so the page row, the new reference and the deletion of the replaced one go through **one** run, and the functional test grants its editors `PAGE_EDIT` without `CONTENT_EDIT` to hold that. Both columns are `exclude` fields, and the field-level grant is asked before the write through the same `BackendUserAuthentication::check('non_exclude_fields', …)` the DataHandler asks it with, because the DataHandler would otherwise create the reference and drop the page's counter in silence — a reference EXT:seo never renders. A translated page is refused and its default-language page named; whether a translation follows its parent or carries its own image stays a page-properties decision — and because core saves a page's translations along with the page, an editor who may not edit a language the page is translated into, or one of its translations itself (a translation carries page permissions of its own), is refused before anything is written, naming the translation, rather than by the DataHandler after the reference row exists. Without EXT:seo the columns do not exist: the `field` enum follows the live TCA and a call is refused naming the extension, since the tool catalogue has no per-tool availability hook to withdraw it. Every surface that counts the writers counts it — ten writers and 51 tools together with `create_record_draft` (ADR-197).

## [0.35.0] - 2026-09-16

### Added

- **A file's title and description can be written at all (ADR-192, `update_fal_asset_meta`).** `set_file_alternative_text` writes one field and refuses every other argument by name, so an assistant asked to caption an asset could set the alternative text and nothing else, and said so — accurately, and uselessly; `read_fal_asset_meta` does not even read `description` back. The eighth writing tool sets `sys_file_metadata.title` and `sys_file_metadata.description` on one file's live default-language record, either field or both, on the terms the previous seven meet: disabled by default, in the `editing` group, `IDEMPOTENT_WRITE`, a human approval before every call, a preview at suspend, a write through the DataHandler as the acting user, a read-after-write verification, and a refusal vocabulary that never confirms a uid exists. It uses the ADR-146 `plan()` shape and is the first `plan()` writer that updates rather than creates, so `createRecord()` does not apply to it. What did get extracted is the FAL lookup, and because it was measured rather than argued: the duplication detector put 93 lines of the new tool against `set_file_alternative_text`, so `ResolvesOneFalAssetTrait` now carries the resolution both metadata writers perform — the `sys_file` row, the storage gate, the default-language access check and the live default-language `sys_file_metadata` row with the three pins that decide which row a write lands on. `set_file_alternative_text` is retrofitted to it and its functional test passes unchanged, which is what makes that a refactor rather than a rewrite. It is a second tool rather than a wider first one because the two are field-disjoint by construction: this one never writes `alternative` and that one never writes these two, so no field of the table has two writers and an approver never has to work out which of two cards won — the cost is two approvals to describe an asset fully, the same trade ADR-180 made for "page plus first element". Two behaviours came out of reading core rather than assuming it. A field the call **leaves out keeps its stored value** while an **empty string clears** the field it names; collapsing those would make "set the title" also erase a description somebody wrote by hand, and three tests fail when the distinction is removed. And core ships `title` with `'exclude' => true` while `description` carries no such flag, so a non-admin granted one and not the other is an ordinary installation — the DataHandler would drop `title` in silence, with an empty `errorLog`, and apply `description`, leaving the asset described by half of an approved call. The grant is therefore asked **before** the write, through the same `BackendUserAuthentication::check('non_exclude_fields', …)` the DataHandler asks it with so the two cannot drift, and the whole call is refused; the per-field read-back stays as the backstop. The title is bounded in **bytes** rather than characters, because the column is `tinytext` and holds 255 of them — a German title of 200 characters is 400 bytes and would be truncated by the database rather than refused.

- **A cancelled tool call is told apart from a failed one (ADR-191, #774).** ADR-190 made a cancelled run stop the MCP call it has on the wire; the result came back as an ordinary error, and that boolean is read all the way to the screen — `RunTrace` writes it onto the step, the payload persists it, `RunTimelineFactory` maps it to `failed`, and the runs module renders that under *Outcome*. So an operator who cancelled a run saw **Failed** beside a server that had answered nothing wrong, and anything counted from those rows counted their own cancel as a fault. A new `@api` enum `ToolOutcome` (`ok`, `failed`, `cancelled`) now travels on `ToolResult`, written by the new `ToolResult::cancelled()` and carried through `withBoundedChannels()` on both of its branches — the error branch rebuilds from almost nothing, and the outcome is the one member that must survive it, or every cancelled call is relabelled a fault before it reaches the audit row (ADR-182 names three values already lost to that shape). `isError` is unchanged and stays true for both non-OK cases, so every consumer reading it keeps its meaning; the outcome says which of the two. One string travels from the enum through the persisted payload to the translation key `runs.detail.outcome.cancelled`, and that equality is asserted rather than assumed. A step written before this keeps the outcome it had: the boolean decides whether a step states an outcome, the new field decides which. `ToolResult::cancelled()`, `ToolResult::$outcome` and `ToolOutcome` are additive on the frozen surface and nothing on it changed shape: `RunTrace::recordToolExecution()` keeps its signature and derives the outcome from the boolean it already took, while the typed `recordToolResult()` reads it off the result. Both build the step through one private method, so `toolIsError` is derived from the outcome rather than passed beside it and the pair cannot disagree; `RunStep` refuses a disagreeing pair, or an outcome on a step that is not a tool step, for every other construction site.

- **A cancelled run stops the MCP call it has on the wire (ADR-190, #774).** Cancelling a run flipped its status and the loop stopped at the next step boundary, but an HTTP request already in flight never learned about it: the run ended while the call ran on to its own deadline — under ADR-170 up to the whole operation budget, about 45 seconds — with an operator watching something they had stopped. A `CancellationSignalInterface` now travels from the tool call into the transport, and nr-vault 0.16's `sendCancellable()` tears the socket down when it turns true. The signal asks the same question the step boundary asks, one indexed read of the run row, so "cancelled" has one definition rather than a second one beside it that can drift; it is throttled to one read per second against nr-vault's ten ticks, monotonic once true, and fail-soft, because a store hiccup must never fabricate a cancellation and an exception thrown mid-transfer would escape after the credential had already gone out. Worst case a cancel is honoured about a second after it is recorded. Feature-detected rather than version-gated: a platform without `curl_multi`, and every caller outside a persisted run — the Tool Playground, any bare `ToolLoopServiceInterface` consumer — takes exactly the blocking send it took before. A cancelled transfer surfaces as its own transport exception rather than as a server that could not be reached, and the model is told the call was cancelled rather than that the tool failed. What is deliberately not decided: whether the remote write landed. Every imported MCP tool declares `NON_IDEMPOTENT_WRITE`, so a torn-down transfer may or may not have mutated something — nothing here retries it (a cancelled run is terminal and its write fence stays stamped), nothing compensates for it, and the operator gets two rows saying what is known: nr-vault's `http_call_cancelled` and the run's own step. ADR-161 recorded this as a gap and pinned it by asserting the seam did not exist; that check is inverted rather than deleted, and now asserts the three places a cancellation may enter and no fourth.

- **Consumers can be told that an AI wrote a record (ADR-187, #896).** A `final readonly` PSR-14 event, `Netresearch\NrLlm\Event\AfterAiRecordWrittenEvent`, is dispatched once per successful editorial write. It carries three values and no fourth: the run's correlation id, the `RecordReference` of the row, and a new `WriteKind` saying whether that row was created or updated. No listener ships here and none is planned — an Article 50 transparency label, a badge in the page module, a line in someone's existing audit trail and a nightly compliance report are four different artefacts with four different owners, and which of them a site wants is the site's decision, not this extension's. The event carries no field values, no before/after and no rendered excerpt either: a payload is a copy, and a copy of editorial content is a second place for it to leak from and to go stale. The reference names the row; a listener reads it under its own permissions when it needs it. Dispatched from the tool loop's single tool-execution choke point, so "exactly once per write" holds for the eighth writer as it does for the seven that exist — and it holds on the resume path too, which is the only path an editorial write ever takes, because every tool declaring a write effect is approval-bound.

- **OpenAI's audio models are discovered as audio models (#913).** `gpt-4o-audio-preview` and its siblings came back from discovery declaring `chat` and nothing else, so `audio` was a capability the enum offered, the TCA rendered and no installation could ever satisfy — a configuration requiring it matched no model at all. `OpenAiModelDiscoverer` now seeds it from the model id, which is the same instrument it already uses for `tts-`, `whisper-` and `gpt-image`, and the only one available: OpenAI's `/v1/models` returns `id`, `object`, `created` and `owned_by`, and nothing that says what a model can do. A model carrying an explicit spec entry is unaffected, because the table wins over the id shape. `audio` stays alongside `chat` rather than replacing it: it names the chat-completions modality — speech in and out of an ordinary chat call — which is a different thing from `text_to_speech` and `transcription`, and those name the dedicated TTS and Whisper endpoints with their own request shape. `json_mode` is deliberately not treated the same way; see below.

### Changed

- **`netresearch/nr-vault` is required at `^0.16.0`** (#774). 0.16.0 adds `CancellableHttpClientInterface` — an outbound send a caller can abort while it is still on the wire — which is the primitive a cancelled agent run needs to stop an in-flight MCP call instead of waiting out its timeout, up to about 45 seconds today. The floor is raised in its own step, ahead of the change under *Added* that uses it, so the version move and the behaviour change stay separable and this step proves by itself that 0.16.0 breaks nothing here. The nr-vault release is purely additive — `VaultHttpClient` gained an interface, no existing signature moved — so the raise is a floor, not a migration. An installation that resolves nr-vault independently needs to allow 0.16.

- **BREAKING: `ToolResult::withWriteTarget()` requires the kind of write.** The method shipped in 0.34.0 taking only a `RecordReference`; it now takes a `WriteKind` beside it, and the frozen surface records both that signature and the new `ToolResult::$writeKind` property. Every caller passes one — the seven builtin writers do, and any consumer-side writing tool must. The parameter is required rather than defaulted because a default would be this extension guessing what a tool did, and the tool is the only party that knows whether it minted the record's uid or was handed it. The pairing is what makes the provenance event above dispatchable without a guess: a target and a kind are set by one method and by no other, so a target without a kind is not a state the value object can be in.

- **BREAKING (behaviour): a conversation is bound to a configuration when it is opened, or it is not opened (ADR-188, #894).** `ConversationService::startSession()` accepted a null configuration and wrote an empty `configuration_identifier`, and everything downstream read that emptiness as "no binding": the ADR-121 context-window fit was skipped, so turn 1 was budgeted against nothing; the turn fell through to the generic `chat()`, where `LlmServiceManager` resolved the installation default itself; and that configuration's skill block was injected into a transcript this service had already sized without it — a limit `skillBlockFor()` documented in its own docblock. So the conversation did run against a configuration, just one nobody wrote down, chosen a layer lower, a turn later, and re-chosen on every turn: switching the installation default halfway through moved a running conversation onto another model, budget and guardrail set, which is exactly what ADR-151's binding exists to prevent. Omitting the argument now means "the installation default, resolved at this moment", persisted with the row. When neither a caller's configuration nor a usable default resolves, the session is refused with `ConfigurationNotFoundException` and **no row is written** — the signature is unchanged, so the frozen surface does not move, but a caller on an installation with no default now fails at `startSession()` instead of one turn later with a different message. Sessions opened before this rule bind themselves once on their next turn — and keep running unbound where the installation still has no usable default, because an improvement that cannot be made is not a reason to end a conversation someone is in the middle of. The binding happens with the actor in hand, so a group-restricted default is evaluated rather than guessed at; no upgrade wizard, because a wizard would write the default as it stood when the wizard ran. The write is conditional on the row still being unbound, and the turn that loses that race follows the row rather than its own read, so the binding holds for the run and not only for the row. `ConfigurationResolver` gains `resolveDefaultForActor()` for that evaluation; it requires a model of a fixed-mode configuration and of nothing else, since a criteria-mode one carries none by design and the resolution every later turn goes through does not ask for one either.

- **BREAKING: `chatForConfiguration()` and `chatWithConfiguration()` accept the call's routing decision (#922).** Both gain a nullable `ModelResolution` as their last parameter, and `ConfigurationCallPlanner::resolveModel()` a nullable `$taken` beside its telemetry signals. Omitting it resolves exactly as before, so every existing caller is unchanged; passing one is how a caller that must size a payload against the serving model avoids forcing a second evaluation. Nullable and last, so the change is breaking only for an implementor of `LlmServiceManagerInterface` — inside this extension there is one, the manager itself. `ModelResolution` moves from `@internal` to `@api` for it and is recorded on the frozen surface: both of its members were already reachable there (`Model` and `RoutingSummary` are both `@api`), so what becomes public is the pairing, not any new data.

- **BEHAVIOUR: a configuration that can serve no chat model refuses a conversation turn before the user row is written (#922).** That refusal (`UnsupportedFeatureException`, from `resolveModelForCall()`'s enforcing branch) used to surface one step later, from inside the dispatch, after the row had been persisted. The row is meant to record a turn that was sent — the "a turn is real regardless of whether the provider then succeeds" rule is about provider failures — and a turn whose configuration matches only models that declare they cannot chat never was one. A provider error after a successful resolution still leaves the user row in place, unchanged.
...
- **BREAKING (for consumers): the configuration resolver is asked by a type, not by a string (#893, second step).** `ConfigurationResolver::getActiveByIdentifier()` and `getActiveByIdentifierForActor()` now require a `ConfigurationIdentifier` — the name of a backend-managed configuration, `blog-summarizer`, which is the third string in this extension returned by a method called `getIdentifier()` and the second one a lookup can silently fail on. `ConfigurationResolver` is not on the frozen surface and appears there only as a constructor parameter type — which means this extension promised nothing about it, NOT that nobody calls it. This entry originally read "nothing outside this extension sees a change", and that was wrong: `netresearch/nr-repurpose` injects the resolver and called `getActiveByIdentifier()` with a plain string, so the narrowing reaches it as a `TypeError` — which its own `catch (NrLlmExceptionInterface)` does not catch, so the fail-soft fallback it keeps for an unresolvable configuration is bypassed and its text pipeline dies rather than degrades. Fixed in netresearch/t3x-nr-repurpose#111, which raises its floor to `^0.35`. Of the sibling extensions checked (`nr-mcp-agent`, `nr-browser-ai`, `nr-ai-search`, `nr-llm-compat`, `nr-repurpose`) that is the only caller, but it is not zero — so a consumer that injects `ConfigurationResolver` must convert at its own call site before upgrading. Inside this extension the one production caller, `ConversationService`, converts at the point where it reads the identifier off the session row. The value object rejects a blank value and trims before it stores, for the reason `ProviderAdapterKey` documents: validating one string and keeping another leaves the padded form constructible, and a padded identifier is a lookup that finds nothing. Because it refuses a blank, the legacy-session bind now normalizes the default configuration's identifier once — before the check and before the write — and declines to bind one that is blank after trimming: a malformed row, since the TCA marks the field required. Otherwise `'   '` passed the check, persisted the sentinel that means *unbound* onto the row, and was refused one line later by the value object that trims before it validates. A per-call pinned provider is still passed to `resolveDefaultConfiguration()` as a string; that one belongs to the provider identifier step, not this one.

### Fixed

- **The governance simulator opened the backend inside the backend.** Both filter forms on the governance page are Extbase GET forms, and a GET submit replaces the query string of the action URI with the form's own fields. The route token lives in that query string — `ExtbaseUriBuilder::buildBackendUri()` unsets the inherited one and lets the backend `UriBuilder` mint a fresh one into the action URI, where the submit then drops it. TYPO3 answers a module route without a token by bouncing it through the backend shell, `/typo3/main?redirect=<route>&redirectParams=<the fields>`, and that shell renders inside the module frame: two headers, two module menus, one page. The token now travels as a hidden field, so the submit reaches the module route directly. The forms stay GET because the controller reads these filters from `getQueryParams()` and because a filter people share as a link should stay in the address bar.

- **"Select a record to start this action" named the precondition and not the path.** The editor-action catalogue is deliberately a read-only overview when it is opened from the module menu — an action needs a subject, and the module has no record picker of its own — and the intro at the top of the page says so, naming the context menu. The hint repeated beside *every* action in the list did not: it said a record was needed, twenty times, while the sentence explaining where to get one sat far above. It now says where to start: right-click a page or content element and choose *AI actions*.

- **Fourteen backend views had no module title, and three had no module menu.** Of the eighteen backend controllers that render a view, ten called `setTitle()` on nothing, nine added no doc-header component, and `AiTaskController` did not even build the module menu — while `LlmModuleController` did all three on every action. A run detail opened from the approvals list therefore arrived with no title, no menu and no way back, which reads as a broken module rather than as a page, and it was reported that way from the demo instance. A `ModuleChromeTrait` now gives every view the same furniture, in the sequence the core itself uses in `SubmoduleOverviewController`: localize the module's own title, set it, build the menu, hand the same identifier and title to the shortcut context. It reads the module off the request rather than taking it as an argument, so the eighteen titles cannot drift from `Configuration/Backend/Modules.php` — a renamed module keeps its header and its bookmark in step by construction, and a view rendered outside a module context gets no chrome instead of a wrong one. Where a controller already sets a richer shortcut than the generic one — `LlmModuleController` carries action arguments on three of its four tabs, so the bookmark returns to that tab — it still sets it afterwards and wins by order.

- **A run decided somewhere else no longer reports that decision's success as this one's failure.** Approving or denying a run in *AI Tasks* answered with *"The run could not be resumed"* whenever the run was no longer waiting — and the commonest way to reach that state is a decision that has just succeeded on another surface. The backend chat in `nr_mcp_agent` offers the same decision on its own card and consumes the run when it is used, so an operator who decided there and then opened the module was told the write had failed, when it had already been performed. Read that way, the message invites deciding again, which is how the same page and content element came to be created twice (nr_mcp_agent NEXT-156, NEXT-153). `RunNotAwaitingApprovalException` and `RunNotAwaitingInputException` now have messages of their own — *"This run is not waiting for an approval any more. It has already been decided, or it has ended."* and its input-side twin — and they are `INFO`, not `WARNING`: nothing went wrong. `runs.flash.error` stays for the case it describes, a resume that genuinely failed.

- **The reranker lock is regenerated for torch 2.14.0 (#926).** `requirements.txt` alone ships nothing — the image installs from `requirements.lock` with `--require-hashes` — so a bump to the former without the latter only makes the two disagree. Regenerated in the pinned `python:3.14-slim` image from the command in the lock's own header: 41 packages, unchanged set, and two of them move — `torch` to `2.14.0+cpu` and the transitive `anyio` to `4.15.1`. The `ci:test:reranker-lock` gate, unreleased as yet and added two entries above, is what caught the gap on the bot's PR rather than letting it reach an image build.

- **The reranker image installed a version the repository did not claim.** `Build/reranker/requirements.txt` and `Build/reranker/requirements.lock` had disagreed since `bad803c9`: that commit raised `sentence-transformers` to 5.7.0 and left the lock at 5.6.1, and the lock is what the Dockerfile installs from, with `--require-hashes`. Every image built in between shipped 5.6.1. Nothing caught it because no workflow builds or reads `Build/reranker` at all. The lock is regenerated (now 6.0.1, verified by a `--require-hashes` resolution of the whole 41-package tree), and `Build/reranker/check-lock-matches-requirements.sh` now runs as part of `composer ci:test:repo` to assert the two files pin the same versions — plain shell, no Docker and no network, so it costs seconds and cannot rot. It sits there rather than in a job of its own because a locally-defined job in `ci.yml` opts out of the shared workflows' action pinning and runner hardening, which is what `ci:test:workflows` exists to prevent — and duly caught the first attempt. Pointed at the state this replaces, it fails with the exact sentence that was missing: `sentence-transformers is 5.7.0 in requirements.txt but 5.6.1 in requirements.lock`.
- **ADR-185 was still marked `Proposed` while six production classes cite it as the decision they implement.** `Documentation/Adr/Index.rst` is explicit about what that status means: "Not yet a decision … Nothing may cite it as settled." The observed-outcome derivation shipped in 0.34.0 and its docblocks say things like "ADR-185 forbids by name" and "ADR-185 states that"; the CHANGELOG entry for that release cites it too. The record was accepted in substance and only its header said otherwise. A sweep of every `Proposed` ADR found this to be the only one that had drifted — ADR-171 is proposed and cited by nothing, which is the state the status describes.
- **A criteria-mode conversation was budgeted against a model it was not sent to (#922).** `ContextWindowManager::fit()` reads `$resolvedModel ?? $configuration->getLlmModel()`, and `ConversationService` never passed the eighth argument — so for a criteria-mode configuration, whose `llm_model` is null by design, the fit fell back to the 8192-token `UNKNOWN_WINDOW_FALLBACK`, while the send resolved a concrete model from the criteria afterwards and fitted again against *its* window. Where the resolved model had the larger window, the first fit had already dropped history the second would have kept, the conversation forgot turns it did not need to forget, and `droppedTurns` recorded that on the user row as if it had been necessary. This predates #894 and ADR-188: every session opened explicitly against a criteria-mode configuration has behaved this way since ADR-121. What #894 changed is the population, because a session opened without a configuration now binds to the installation default and takes this path too. The turn now takes **one** routing decision, in `ConversationService`, and carries it to both halves: the resolved model goes to the fit, and the whole `ModelResolution` travels with the dispatch, so the manager's terminal records its routing summary and evaluates nothing a second time. Two evaluations moments apart can disagree about which model a turn runs on — the hazard `ConfigurationCallPlanner::resolveModel()` names in its own docblock — and one decision per turn is the only shape that cannot. The recording order ADR-174 fixes is unchanged: request facts are still formed before the pipeline, and the routing summary is still recorded from inside the terminal; only the evaluation moved. The handed-over decision is scoped to the configuration it was taken for — `FallbackMiddleware` retries through `ProviderCallContext::withConfiguration()`, so an attempt may be serving a configuration the caller never named, and reusing the primary decision there would send the fallback to the primary model, its adapter and its window, which is the one thing a fallback exists to avoid. A fixed-mode configuration is unaffected — it named its model, and both fits already used it.
- **A capability the enum declares but no discoverer assigns now fails the build.** `ModelCapabilityRegistrationTest` held the enum against its TCA items and its label files; it did not hold it against the discovery side, and that is where the expensive drift was. `json_mode` and `audio` were both declared, offered in the Models module and requirable in a configuration's criteria while no discoverer assigned either, so `EligibilityEvaluator::matchesCapabilities()` matched no model at all for a criteria set naming one. (`audio` is assigned as of the entry above; `json_mode` is the one that stays unassigned.) That is not hypothetical: `nr_repurpose_text` asked for `json_mode`, and the use-case pack carrying it could not be installed on any installation until the criterion was dropped. The known cases are listed with their reason rather than hidden, and a second assertion fails when a discoverer starts assigning one, so the list cannot outlive it — `audio` left the list the same day it was written, which is the mechanism working (#913).
- **A preset preflight names the capability that fails, not the whole requested set.** `capabilities: chat, json_mode` was the message an operator got when the criteria matched nothing, and it points at `chat`, which every model on the installation declares. Only `json_mode` was actionable. The preflight now narrows one capability at a time and reports the ones no active model declares — `capabilities: json_mode (declared by no active model)`. When each capability matches something alone and only the combination fails, it says that instead, because that is a different problem with a different fix. The remedy the result carries is unchanged; what changes is that the sentence sends the reader to the record they have to edit (#913).
- **`json_mode` is documented as the one capability that stays unassigned, and why (#913).** It is not the same kind of thing as the ten around it: it is a request option, not something a model can be asked to do. `ChatOptions` carries `responseFormat` to the provider unconditionally and no call path consults the capability, so as a selection criterion it can only ever subtract models — which is the incident that opened the ticket. Populating it would mean asserting per model that the provider honours `response_format`, and a declared capability the provider does not honour fails at call time instead of at selection time, which is worse than leaving it empty. Whether the case is removed outright is left to the 1.0 API freeze (#895), because the enum case and `Model::supportsJsonMode()` both sit on the frozen surface.
- **The dead-capability guard reads what the discoverers write, not what their sources say (ADR-189).** The check scanned `Classes/Service/SetupWizard/Discovery` for the quoted capability token, which cannot tell an assignment from an array subscript: OpenRouter's `$pricing['completion']` counted as evidence that `completion` was seeded, so the guard stayed silent about a capability no discoverer writes — a third case, beside `json_mode` and the `audio` that has since been seeded. It now lives in `CapabilitySeedTest`, runs every discoverer against its offline corpus and the recorded responses, and collects what actually lands in `DiscoveredModel::$capabilities`, where a subscript cannot be mistaken for a seed. `ModelCapability` documents per case who writes it and who reads it — including that `completion` is a synonym of `chat` here, since `CompletionService::complete()` delegates to `LlmServiceManager::chat()` and every adapter posts to `chat/completions`. ADR-189 states the rule the three surfaces had been following unevenly: criteria may require only what some producer writes. `OperationCapabilityMap` said `embeddings` was written by no discoverer; Gemini and Ollama both write it, and that paragraph is the argument for what gets enforced, so it is corrected (#913).
...
- **An approval now binds to the state its preview showed (ADR-184).** ADR-136 refused this fence on the premise that "a writing tool here sets absolute values", and named what would reopen it: a writer whose change is relative. `attach_file_to_content_element` shipped ten days later and appends — its preview reads `field image: 2 reference(s) → 3, appended last`, and both halves of that line are operands of the write rather than decoration on it. When a run resumes, the loop re-runs the preview of every approved call and compares it line-for-line with what the approver was shown. Equal, the call executes as before. Different, and nothing is written: the run suspends for approval again, carrying the CURRENT preview and a marker naming the call that moved, and the card says why. The mechanism needs no new fingerprint — `SuspendedRunState` has stored the preview lines since ADR-136, so the preview IS what the approval is bound to. Only what was displayed counts: a field the preview never showed can change freely. A preview that failed at suspend binds nothing (the approver was told they were deciding blind); one that cannot be recomputed at resume refuses, because unknown is not equality. `ToolPreviewInterface` gains the contract that makes this work — a preview must be a pure function of the subject's state and the call's arguments, or every approval of that tool stales forever. `SuspendedRunState` gains one optional constructor parameter and the readonly property behind it, which is additive on the frozen surface (#887, #888).

- **A write tool names the record it wrote** — `ToolResult` gains an optional `writeTarget`, a two-scalar `RecordReference` (table and uid), and all seven writers now set it on success. The uid a write touched used to exist only as whatever the model had put in the tool arguments — free-form JSON, not a fact the runtime had established — so `ACCEPTED_UNCHANGED`, `EDITED` and `DISCARDED` had nothing to join `sys_history` against and `CallOutcome::isImplemented()` returned false for exactly those three. The run trace persists the target as a step of its own kind, `tool_write`, carrying an identity and nothing the record holds; it survives every privacy level for that reason, and the run timeline shows it so an operator can go and look at what the run actually changed. A read-only tool that claims a write target, and a declared writer that names no record, both fail `ToolEffectCoverageTest`. ADR-182 discharges the part of ADR-122 that now has a reader; the idempotency scope and the preview contract stay deferred with their reasons intact (#889, #890, #891, #892).
...
- **A pack may ship snippets its own extension reads, and give them metadata (ADR-186).** `PackSnippet` gains two optional fields, both defaulting to today's behaviour. `metadata` is stored as the record's `metadata` JSON object — the column that already carries a persona's `voice` and a layout's `imageSize`, which a pack could not deliver at all before, so it could ship a persona's text and not its voice. `composedByConfiguration: false` declares a snippet the declaring extension resolves by uid instead of leaving it to ADR-031 tag composition; `UseCasePack::getSnippetTags()` skips those, so no tag is linked and the plan screen reports no configuration they reach. The flag sits on the snippet rather than the pack because that is where the property is — a snippet either is or is not meant to be read by tag, and a pack is the bundle it arrives in. The first consumer opts out for all five of its tags: `nr_repurpose` resolves audience and tone per job exactly as it resolves persona, layout and style, and linking `persona` would append every active persona to the system prompt of every completion on that configuration, on top of the one the editor picked for the job.
- **`nrllm:usecasepack:install <identifier>` — a pack without a backend session.** The Use Case Packs module shows a plan and waits for a confirmation, which a container entrypoint, a `make update` step or a CI provisioning run cannot give. The command writes through the same installer, so it creates only what is missing, and re-running is a no-op that reports what was already there — which is what makes it safe in a deploy step that runs every time, and the answer to an instance rebuilt from a database seed that predates the pack. An unknown identifier fails and lists the declared ones. Like the module, it enables no tool group, enables no editor action and applies no governance profile (ADR-168).
...
- **Thirteen `if (!isset(…))` assignments became `??=`.** `rector/rector` 2.6.5 widened `IfToNullCoalescingAssignRector`, and because this repository does not commit `composer.lock`, CI resolved the new release and the Rector gate went red on `main` without a line of ours moving — the same shape as #898 one release earlier, and it blocks every open pull request until it is fixed. Unlike that case the proposal is correct: `??=` assigns exactly when `isset()` is false, so the rewrite is behaviour-preserving, and applying it is cheaper than carrying an exemption.
...
- **`AgentEventKind` knows the two kinds it is written with.** `dropped` has been persisted to `tx_nrllm_agentrun_event.kind` since ADR-179 and `tool_write` since ADR-182, and neither had a case. Nothing broke, which is why nobody noticed — the repository inserts the raw string with no validation — but `AgentRunEvent::kindEnum()` answered null for both, and anything filtering an event stream by typed kind silently lost exactly those rows. Two functional suites do that today, and the observed-outcome reader (#772) is going to join against one of the two. The enum now carries both, and a test holds it against `RunStep`'s own `KIND_*` constants by reflection, in both directions — so the next kind added fails there instead of three records later (#900).

- **`ProviderConnectionTest` no longer measures the runner.** Three of its cases asserted a wall-clock bound around a call to a non-routable address. They passed when run alone and failed under the four-way sharded functional run that gates every pull request — where the failure would have read as a provider regression on an unrelated change. A timing assertion on a shared runner measures the runner. What the bound was protecting is now asserted where no socket and no clock are involved: a unit test drives the registry with an adapter double whose connection always fails, and pins that the failure becomes a RESULT the backend can render rather than an exception the caller must catch. A second case on the same path pins something that had no test at all — a connection failure quotes the URL it failed on, and for a provider whose key travels in the query string that URL carries the key into a message the backend renders (#868).
...
- **The tool loop no longer rebuilds a tool's result from two of its properties.** `ToolLoopService` answered with `ToolResult::text($boundedContent, ...$boundedArtifacts)`, which dropped anything else the result carried — and every test asserting on the tool's own `execute()` return value would still have passed. It is the third time this shape has cost a value here (#844, #845, #846), so bounding is now a transformation that carries every other member forward, and a property added to `ToolResult` without a decision fails a test instead of vanishing (#890).
...
- **The specialized services attribute their spend.** Image generation, speech synthesis, transcription and DeepL translation inherit `withCallerSource()` from `AbstractOptions`, so a consumer could annotate them and the call compiled — and the value was then dropped, which is the worst shape a declaration can have. Two places lost it: `fromArray()` rebuilt the options object from its own keys only, and the services build their own `ProviderCallContext` instead of going through `LlmServiceManager`, where the identity is normally translated into call metadata. Both are closed, so `TelemetryMiddleware` persists `source_extension` / `source_operation` and `UsageMiddleware` aggregates the cost row on the same key (ADR-177/ADR-178). Image generation is among the most expensive operations an installation can trigger and was the line item that would have stayed in *Unattributed* while every text call moved out of it (#844).
