...
- **An approval now binds to the state its preview showed (ADR-184).** When a run resumes, the loop re-runs the preview of every approved call and compares it line-for-line with what the approver was shown. Equal, the call executes as before. Different, and nothing is written: the run suspends for approval again, carrying the CURRENT preview and a marker naming the call that moved, and the card says why. The mechanism needs no new fingerprint — `SuspendedRunState` has stored the preview lines since ADR-136, so the preview IS what the approval is bound to. Only what was displayed counts: a field the preview never showed can change freely. A preview that failed at suspend binds nothing (the approver was told they were deciding blind); one that cannot be recomputed at resume refuses, because unknown is not equality. `ToolPreviewInterface` gains the contract that makes this work — a preview must be a pure function of the subject's state and the call's arguments, or every approval of that tool stales forever. `SuspendedRunState` gains one optional constructor parameter and the readonly property behind it, which is additive on the frozen surface (#887, #888).

- **ADR-184 reopens the stale-approval question, because the tool ADR-136 named as its trigger now exists.** ADR-136 refused to fence an approval on its target's state, on the premise that "a writing tool here sets absolute values" — and said the record should be revisited when one wrote a relative change. `attach_file_to_content_element` shipped ten days later and appends: its preview reads `field image: 2 reference(s) → 3, appended last`, and both halves of that line are operands of the write rather than decoration on it. The record proposes binding an approval to the values its preview displayed, with a re-preview and a second pause instead of a dead end — which is the answer to the strongest objection ADR-136 raised. Proposed only; nothing changes behaviour yet (#887, #888).
