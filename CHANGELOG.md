...
- **The configuration resolver is asked by a type, not by a string (#893, second step).** `ConfigurationResolver::getActiveByIdentifier()` and `getActiveByIdentifierForActor()` now require a `ConfigurationIdentifier` — the name of a backend-managed configuration, `blog-summarizer`, which is the third string in this extension returned by a method called `getIdentifier()` and the second one a lookup can silently fail on. `ConfigurationResolver` is not on the frozen surface and appears there only as a constructor parameter type, so nothing outside this extension sees a change; the one production caller, `ConversationService`, converts at the point where it reads the identifier off the session row. The value object rejects a blank value and trims before it stores, for the reason `ProviderAdapterKey` documents: validating one string and keeping another leaves the padded form constructible, and a padded identifier is a lookup that finds nothing. Because it refuses a blank, the legacy-session bind now declines to bind a default configuration whose identifier is empty — a malformed row, since the TCA marks the field required — instead of writing the sentinel that means *unbound* onto the row and re-binding on every turn. A per-call pinned provider is still passed to `resolveDefaultConfiguration()` as a string; that one belongs to the provider identifier step, not this one.
