...
- **`create_translation_draft` machine-translates the text of the translation (ADR-209, NEXT-166).** After core's `localize` created and hid the record, every text column of the record's type that holds text in the source — `input` and `text` columns from the TCA with the type's `columnsOverrides`, without `l10n_mode = exclude`, `defaultAsReadonly`, `readOnly` or a `text` column rendered in a code editor — is translated from the default-language record (so the "[Translate to …:]" prefix never reaches the translator) and written into the translation in one DataHandler pass as the acting user. Rich text is sent as HTML. The new optional `translator` argument chooses `deepl` or `llm` (default `llm`, TranslationService's own default without a configuration); a translator the installation has not configured is refused before anything is created. The site of the record's page selects the site glossary (ADR-208). The approval preview names the translator and the fields; columns the user may not edit are named and left out. When a translation, the write or its read-back fails, nothing is written and the result says *The text was NOT machine-translated* with the reason — the draft keeps the copied source text, and the result still carries the write target, so `AfterAiRecordWrittenEvent` announces the created record. A transparency-notice field for machine-translated records was not added: this extension does not extend `pages` or `tt_content`, and ADR-187 ships no listener; ADR-209 records why.

- **Opt-in translation cache and markup handling in `TranslationOptions` (ADR-209, NEXT-166).** `withCacheTtl(int)` caches the result of `TranslationService::translateWithTranslator()` in `nrllm_responses` (tag `nrllm_translation`), for DeepL and the LLM translator alike. The key is the translator, both languages, the text and the final translator options — including the site glossary's terms or DeepL glossary id, so an edited glossary is a new key — without the attribution fields. A hit skips the translator, its budget pre-flight and its usage row, carries `metadata['cached'] = true` and `charactersUsed = 0`; a failure is never stored; without a TTL nothing changes. `withTagHandling('html'|'xml')` reaches DeepL as `tag_handling`. Both are appended to the constructor with getters; `toArray()` emits `tag_handling`. `TranslationService`'s constructor gains an optional trailing `CacheManagerInterface`; positional callers are unaffected.
