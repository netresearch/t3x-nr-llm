...
- **A record in an extension table can be drafted where no dedicated writer exists (ADR-197, `create_record_draft`).** On the Netresearch demo an assistant recognised EXT:news and its table and could not create a news record: every writer is narrow (ADR-135), and no narrow writer will exist for every extension an installation carries. The new builtin creates ONE hidden record in a TCA table by exclusion rather than by allow-list, and every exclusion is part of the decision: `pages` and `tt_content` have writers of their own and are refused by name; `sys_*` tables, the read-side denylist (`TableReadAccessService`) and tables declared `adminOnly`, `hideTable` or `readOnly` are out; a table another registered writer declares through its editor-action record types is refused and the writer named — read at call time from the `nr_llm.tool` tagged set, so an extension that ships its own writer withdraws the fallback the day it is installed, without a release here; and the new extension configuration `tools.createRecordDraft.deniedTables` (comma-separated, empty by default) narrows the list per installation and can never widen it, refusing every table when it cannot be read. Only scalar columns the record type's showitem lists — palettes expanded — can be set (`input`, `text`, `number`, `email`, `color`, `datetime` as a timestamp, `check`, `radio`, `select` with static items); relations, files, FlexForms, links and slugs are not arguments, and `hidden`, `uid`, `pid`, the language, timing, ownership and versioning columns are refused by name. Values are checked against the live TCA before anything is written — the items of a select the way ADR-194 checks one, `max`, `range`, 0/1 for a check — and every column the TCA marks required must be given, because the DataHandler drops an empty required value and clamps an out-of-range one in silence. The record is always hidden and always in the default language: a translation needs a parent and a tool of its own, and a standalone record in another language is what ADR-193 found to mix a page. Rights are the acting user's, asked before the write — `tables_modify`, `Permission::CONTENT_EDIT` on the pid (what `DataHandler::hasPermissionToInsert()` asks for every table but `pages`), the field-level grant for every column set, the hidden column included — and verified after it: what the DataHandler still rewrites in silence, a `min` it resets, a hook, is read back column by column, the record is deleted again and the columns are named. It declares no editor action, because a declaration names the tables a writer owns and this one owns none.
