...
- **`netresearch/nr-vault` is required at `^0.16.0`** (#774). 0.16.0 adds `CancellableHttpClientInterface` — an outbound send a caller can abort while it is still on the wire — which is the primitive a cancelled agent run needs to stop an in-flight MCP call instead of waiting out its timeout, up to about 45 seconds today. Nothing in this release uses it yet: the floor is raised on its own so that the version move and the behaviour change are separable, and so this step proves by itself that 0.16.0 breaks nothing here. The nr-vault release is purely additive — `VaultHttpClient` gained an interface, no existing signature moved — so the raise is a floor, not a migration. An installation that resolves nr-vault independently needs to allow 0.16.
