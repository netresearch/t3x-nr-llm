...
- **The assistant can act on existing pages and content elements (ADR-198, ADR-199, NEXT-164).** NEXT-160 had kept updating, publishing and deleting out of reach as a security line to decide separately; the product owner decided on 2026-09-23 to include them, and ADR-198 records the decision and its rails. Six new writing tools, each disabled by default, in the `editing` group, `IDEMPOTENT_WRITE` or `NON_IDEMPOTENT_WRITE`, paused for a human approval on every call, writing through the DataHandler as the acting backend user in the live workspace only: `update_content_element` sets the scalar fields of one existing element — the field set `create_content_element_draft` offers for the element's type, checked by the same ADR-196 rules, which move unchanged into a shared `ReadsContentTypeFormsTrait` — and refuses an element of an excluded type (raw HTML, plugins, menus, shortcuts, FlexForm or inline forms), a column a translation takes from its default-language element, and what the page's TSconfig takes out of the form; the approval card shows every column before and after. `publish_record` clears the hidden flag of one page or element and nothing else, and names the start or stop time, access groups or hidden default-language record that still restrict it. `delete_record` deletes one page or element, recoverably, and counts what core deletes with it — translations, a page's records and its branch — and the records that still point at it; a page with subpages is refused unless `include_subpages` is set, a branch over 50 pages and a site root always. `copy_record` copies one element or page with core's copy command — the copy and every translation copied with it always hidden, whatever `hideAtCopy`, the user's preferences and page TSconfig say — and a page always without its subpages (ADR-199). `move_page` moves one page to a new parent or after a sibling with the permissions core's `moveRecord()` asks, and says that the slug is not regenerated. `replace_file_reference` points one file reference on a content element at another existing file at the same position, or removes it; nothing of the old reference — title, alternative text, description, crop — is carried over. Every tool asks the acting user's page permission for its act, the record-level rights core asks (`tables_modify`, language, `authMode`, `editlock`), the languages of translations core carries along, and the field-level grant for every column it sets, before the write; it reads the result back after it. None declares an editor action: from a record's context menu and in bulk, a delete would be one click, which is a decision of its own. `WriteKind` gains `DELETED` for the first deleting writers; the API surface snapshot gains the case, and a consumer that `match`es the enum exhaustively needs an arm for it. Every surface that counts the tools counts 57 tools and sixteen writers.
