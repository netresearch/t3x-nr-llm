...
- **A criteria-mode conversation was budgeted against a model it was not sent to (#922).** `ContextWindowManager::fit()` reads `$resolvedModel ?? $configuration->getLlmModel()`, and `ConversationService` never passed the eighth argument — so for a criteria-mode configuration, whose `llm_model` is null by design, the fit fell back to the 8192-token `UNKNOWN_WINDOW_FALLBACK`, while the send resolved a concrete model from the criteria afterwards and fitted again against *its* window. Where the resolved model had the larger window, the first fit had already dropped history the second would have kept, the conversation forgot turns it did not need to forget, and `droppedTurns` recorded that on the user row as if it had been necessary. This predates #894 and ADR-188: every session opened explicitly against a criteria-mode configuration has behaved this way since ADR-121. What #894 changed is the population, because a session opened without a configuration now binds to the installation default and takes this path too. The service now asks which model the turn will run on and passes it to the fit. It asks through `explainRouting()` rather than `resolveModelForCall()`, deliberately: the authoritative resolution belongs to the manager's terminal where ADR-174 puts it, and it is the one that records an observed capability mismatch or refuses an unservable operation — a second reporting resolution here would double-count the observation and move the refusal to a different layer, whereas `explainRouting()` runs the same constrained decision without either side effect. A failure of that lookup is logged and falls back to the previous behaviour rather than ending a conversation, because it is a budget estimate and the real resolution downstream raises whatever is actually wrong. `RoutingReadout` gains `selectedModel()` for it, which is `hasSelection()`'s branch answering with the model instead of with whether there is one. A fixed-mode configuration is unaffected: it named its model, and both fits already used it.
