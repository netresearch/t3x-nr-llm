...
- **`CHANGELOG.md`**: append under the heading that already exists in `## [Unreleased]` — inserting above the first `###` heading produces a duplicate one. When merging `main` into a series of sibling branches, rebuild the section (take the incoming version verbatim, re-insert your own block) instead of trusting the merge. `composer ci:test:changelog` (pre-commit and CI) refuses the repeated result. **It cannot tell you the entry is in the wrong SECTION**, and that is the more expensive failure: it reads only from `## [Unreleased]` to the next `## [`, by design — released sections are history to it. Rebase a branch across a release and your Unreleased entry can land inside the published section, describing a change that release does not contain, with the check still green. After any rebase that crosses a release boundary, look at where your entry sits before trusting the gate.
