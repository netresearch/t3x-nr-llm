...
| A subset of a suite | `./Build/Scripts/runTests.sh -s unit -- --filter '<regex>'` — the runner has short options only; everything after `--` goes to the tool (there is no `-e`) |
...
| E2E (Playwright) | `ddev e2e` (supplies the URL; `TYPO3_BASE_URL=… ./Build/Scripts/runTests.sh -s e2e` without ddev) |
...
- **`CHANGELOG.md`**: append under the heading that already exists in `## [Unreleased]` — inserting above the first `###` heading produces a duplicate one. When merging `main` into a series of sibling branches, rebuild the section (take the incoming version verbatim, re-insert your own block) instead of trusting the merge. `composer ci:test:changelog` (pre-commit and CI) refuses the repeated result. An entry can also land in the wrong SECTION — inside a published one, describing a change that release does not contain. A rebase across a release does that, and so does the merge queue when it combines a PR with a release commit (#970's entry landed in `[0.37.0]` that way, with no rebase at all). `check-changelog-released-sections.php`, part of the same composer script, catches it: every released section must equal the section at its tag (`git show vX.Y.Z:CHANGELOG.md`); a tag missing in a shallow checkout is fetched, and only the topmost section may precede its tag. A deliberate correction of a released section goes into its `KNOWN_DEVIATIONS` list with a reason.
