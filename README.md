...
[![OpenSSF Baseline](https://www.bestpractices.dev/projects/11697/baseline)](https://www.bestpractices.dev/en/projects/11697/baseline-1)
...
- **Tools** — 42 built-in read-only function-calling tools plus sixteen writing tools, in 10 toggleable groups — `content` (search, page content, record history: "who changed this?"), `structure` (TCA/FlexForm schema, URL resolution, TCA validation), `configuration` (TypoScript/TSconfig, syntax check, site config), `code` (last exception, source read/search), `files` (FAL browse/search/references), `system` (URL probe, diagnostics), `accounts` (BE users/groups), `rag` (cited site-content evidence from the installed search index — EXT:solr, ke_search, indexed_search or a database fallback), `web` (`fetch_external_url`: reads one public web page on another site through an address guard; ships disabled) and `editing` (`update_page_metadata`: sets descriptive fields on one page through the DataHandler; `set_page_social_image`: sets the Open Graph or Twitter image of one page to an existing file; `set_file_alternative_text`: sets the alternative text of one managed file; `update_fal_asset_meta`: sets the title, description and copyright of one managed file; `attach_file_to_content_element`: references an existing file from one content element; `move_content_element`: moves one element to another page or column; `create_content_element_draft`, `create_page_draft` and `create_translation_draft`: create a HIDDEN element, page or translation a human must unhide; `create_record_draft`: creates a HIDDEN record in an extension table no dedicated writer covers; `update_content_element`: changes the scalar fields of one existing content element; `publish_record`: clears the hidden flag of one page or content element; `delete_record`: deletes one page or content element, recoverably; `copy_record`: copies one element or page, always hidden and without subpages; `move_page`: moves one page to another parent; `replace_file_reference`: swaps or removes one file reference on a content element — all sixteen ship disabled, and every call pauses for a human approval). See the [Administration guide](Documentation/Administration/Tools.rst).
